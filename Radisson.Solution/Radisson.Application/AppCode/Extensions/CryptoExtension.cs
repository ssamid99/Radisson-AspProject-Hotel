...
namespace Radisson.Application.AppCode.Extenstions
{
    public static partial class Extension
    {
        const string saltKey = "final@@RadissonHotel!";
        public static string ToMd5(this string value)
        {
            byte[] buffer = Encoding.UTF8.GetBytes($"{saltKey}{value}Radisson.Application.AppCode.Extensions");
        ...
        {
            byte[] buffer = Encoding.UTF8.GetBytes($"{saltKey}{value}2Radisson.Application.AppCode.Extensions");
        ...
        {
            byte[] buffer = Encoding.UTF8.GetBytes($"{saltKey}{value}1Radisson.Application.AppCode.Extensions");
        ...
        public static string Encrypt(this string value, string key, bool appliedUrlEncode = false)   //123
        {
            try
            {
                ...
                {
                    var keyBuffer = md5.ComputeHash(Encoding.UTF8.GetBytes($"#{key}!2023"));
                    var ivBuffer = md5.ComputeHash(Encoding.UTF8.GetBytes($"2023@{key}$"));
